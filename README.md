...
See `update-from-template-prompt.md` at the repo root. Paste it into a PM thread to fetch the template, compare file-by-file, and apply the relevant changes per the conflict-resolution table documented there.

Recommended cadence: monthly, or when you are about to set up a new integration and want to see if the template has a fresh reference doc for it.
